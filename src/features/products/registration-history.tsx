...
const generateMock = (count = 40) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    rows.push({
      seq: i + 1,
      source: `${PLATFORMS[i % PLATFORMS.length]} 상품등록`,
    ...
    <Container maxWidth="full" centered={false} padding="md">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="h-2 bg-gray-100 rounded overflow-hidden w-64">
                        <div
                          ...
                          style={{
                            width: `${r.total ? Math.round((r.success / r.total) * 100) : 0}%`,
                          }}
                        />
                      </div>
                      {r.failed > 0 && (
                        <div className="text-xs text-white bg-red-500 rounded px-2 py-0.5">
                          {r.failed}
                        </div>
                      )}
                    </div>
                    <div className="text-xs text-gray-600 mt-1">
                      성공 {r.success} / 실패 {r.failed}
                    </div>
                  </td>
                  <td className="px-3 py-2 align-top">
                    <Button
      ...
      {openSeq !== null && (
        <Modal
          ...
          size="big"
          title={`입력차수 ${openSeq} - 상품 목록`}
          footer={
            <Button variant="secondary" onClick={() => setOpenSeq(null)}>
              닫기
            </Button>
          }
        >
