...
const OrderSlipListPage: React.FC = () => {
  ...
  const filteredOrderSlips = mockOrderSlips.filter((slip) => {
    const matchesStatus =
      selectedStatus === "all" || slip.status === selectedStatus;
    const matchesSupplier =
      selectedSupplier === "all" || slip.supplierId === selectedSupplier;
    const matchesSearch =
      searchTerm === "" ||
  ...
  const getTotalStats = () => {
    return {
      ...
      confirmed: filteredOrderSlips.filter((s) => s.status === "confirmed")
        .length,
      shipped: filteredOrderSlips.filter((s) => s.status === "shipped").length,
      delivered: filteredOrderSlips.filter((s) => s.status === "delivered")
        .length,
      totalAmount: filteredOrderSlips.reduce(
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredOrderSlips.map((slip) => (
                <tr key={slip.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">{getStatusBadge(slip.status)}</td>
                  <td className="px-4 py-3">
                    <div className="text-sm text-gray-900">
                      {slip.productCount}종 / {slip.totalQuantity}개
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    ...
                    <div className="flex space-x-2">
                      ...
                      {slip.status === "pending" && (
                        <>
                          <button
                            onClick={() =>
                              handleStatusChange(slip.id, "confirmed")
                            }
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-5xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-semibold">
                발주전표 상세 - {selectedSlip.orderNumber}
              </h2>
              <button
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="text-lg font-bold text-blue-600">
                    ₩{selectedSlip.totalAmount.toLocaleString()}
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">
