...
function loadTreeFromApi(): Promise<any[]> {
  return fetch("/api/meta/classifications")
    .then((r) =>
      r.ok
...
function findAndUpdate(
  nodes: TreeNode[],
  id: string,
  updater: (n: TreeNode) => TreeNode | null,
): TreeNode[] {
  return nodes
    .map((n) => {
      if (n.id === id) {
      ...
      if (n.children) {
        const nextChildren = findAndUpdate(n.children, id, updater).filter(
          Boolean,
        ) as TreeNode[];
...
function appendChild(
  ...
  return nodes.map((n) => {
    ...
    if (n.children)
      return { ...n, children: appendChild(n.children, parentId, child) };
    return n;
  });
}
...
function renderNodes(
  ...
  nodes.forEach((n) => {
    ...
    const accent =
      accentColors[Math.max(0, Math.min(accentColors.length - 1, depth - 1))];
          ...
          {hasChildren && (
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                setExpanded &&
                  setExpanded((s) => ({ ...s, [n.id]: !isExpanded }));
              }}
...
export default function ProductCategoryPage() {
    ...
    loadTreeFromApi()
      .then((apiTree) => {
        ...
        if (apiTree && apiTree.length > 0) {
          ...
          try {
            window.localStorage.setItem(STORAGE_KEY, JSON.stringify(apiTree));
          } catch (e) {}
        } else {
          const t = loadTree();
          setTree(t);
  ...
  const openAddRoot = () => {
    setEditing({ node: null, parentId: null });
    setParentPath(null);
    setName("");
    setSlug("");
    setModalClass("max-w-3xl");
    setShowModal(true);
  };
  const openAddChild = (parent: TreeNode) => {
    setEditing({ node: null, parentId: parent.id });
    setParentPath(null);
    setName("");
    setSlug("");
    setModalClass("max-w-2xl");
    setShowModal(true);
  };
  const openEdit = (node: TreeNode) => {
    setEditing({ node, parentId: null });
    setParentPath(null);
    setName(node.name);
    setSlug(node.slug ?? "");
    setModalClass("max-w-xl");
    setShowModal(true);
  };
  ...
  const save = () => {
    ...
    if (editing.node) {
      // edit existing
      const next = findAndUpdate(tree, editing.node.id, (n) => ({
        ...n,
        name: name.trim(),
      ...
      if (targetParent) {
        ...
        if (parentDepth + 1 > MAX_DEPTH)
          return alert(`최대 ${MAX_DEPTH}뎁스까지만 생성할 수 있습니다`);
      }
    ...
    <Container>
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-600">
              상품 카테고리를 추가, 편집 및 삭제합니다. 트리 구조를 지원합니다.
            </p>
          </div>
          <div>
            <button
            ...
            >
              <div
                style={{
                  width: 1,
                  height: "100%",
                  background: idx === 0 ? "#e6eefc" : "#eef3fb",
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div
            className={`bg-white rounded-lg shadow-lg w-full ${modalClass}`}
            style={{ maxHeight: "80vh", overflow: "auto" }}
          >
            <div className="flex items-center justify-between p-4 border-b">
              <div>
                <h3 className="text-lg font-semibold">
                  {editing && editing.node ? "카테고리 편집" : "카테고리 추가"}
                </h3>
                <div className="text-sm text-gray-500">
                  최대 {MAX_DEPTH}뎁스까지 생성할 수 있습니다.
                </div>
              </div>
              <button
              ...
              <div>
                {!editing?.node ? (
                  ...
                  <div className="text-sm text-gray-600">
                    편집 모드입니다. 위치 변경은 카테고리 삭제 후 재생성으로
                    처리하세요.
                  </div>
                )}
              </div>
          ...
          >
            ...
            <div className="p-4">
              <div className="text-sm font-medium mb-2">
                삭제 대상: {deleteTarget.name}
              </div>
              <div className="text-xs text-gray-600 mb-4">
                하위 항목 {deleteTarget.paths.length}개 포함
              </div>
              <div className="max-h-64 overflow-auto border rounded p-2">
                {deleteTarget.paths.length === 0 && (
                  <div className="text-sm text-gray-500">
                    하위 항목이 없습니다.
                  </div>
                )}
                <ul className="text-sm space-y-1">
                  {deleteTarget.paths.map((p, idx) => (
