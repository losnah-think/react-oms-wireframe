...
const ExpectedDeliveryListPage: React.FC = () => {
  ...
  const isOverdue = (expectedDate: string) => {
    return new Date(expectedDate) < new Date() && new Date().toDateString() !== new Date(expectedDate).toDateString();
  };
  ...
  const filteredDeliveries = mockExpectedDeliveries.filter(delivery => {
    ...
    const matchesSupplier = selectedSupplier === 'all' || delivery.supplierId === selectedSupplier;
  ...
  const handleReceiveStock = (delivery: ExpectedDelivery) => {
    ...
    const items = mockExpectedDeliveryItems.filter(item => 
      item.expectedDeliveryId === delivery.id && item.remainingQuantity > 0
    );
    setReceiveItems(items.map(item => ({...item, receivedQuantity: item.remainingQuantity})));
  ...
  const handleUpdateReceiveQuantity = (itemId: string, quantity: number) => {
    setReceiveItems(items => items.map(item => 
      item.id === itemId ? { ...item, receivedQuantity: Math.min(quantity, item.remainingQuantity) } : item
    ));
  };
  ...
  const getTotalStats = () => {
    return {
      ...
      totalExpectedAmount: filteredDeliveries.reduce((sum, d) => sum + d.estimatedAmount, 0),
      totalReceivedAmount: filteredDeliveries.reduce((sum, d) => sum + d.receivedAmount, 0)
    };
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-600">발주된 상품들의 입고 예정 현황을 확인하고 입고 처리를 진행합니다.</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-7 gap-4 mb-6">
        ...
        <div className="bg-white border rounded-lg p-4">
          <div className="text-2xl font-bold text-indigo-600">₩{(stats.totalExpectedAmount / 1000000).toFixed(1)}M</div>
          <div className="text-sm text-gray-500">예정금액</div>
        </div>
        <div className="bg-white border rounded-lg p-4">
          <div className="text-2xl font-bold text-purple-600">₩{(stats.totalReceivedAmount / 1000000).toFixed(1)}M</div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">진행률</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">수량정보</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">금액정보</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">배송정보</th>
              ...
              {filteredDeliveries.map((delivery) => (
                <tr key={delivery.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3">
                    ...
                    <div className="text-xs text-gray-500">발주일: {delivery.orderDate}</div>
                    {delivery.memo && (
                      <div className="text-xs text-gray-500 mt-1">📝 {delivery.memo}</div>
                    )}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div className="text-sm text-gray-900">{delivery.expectedDeliveryDate}</div>
                    {isOverdue(delivery.expectedDeliveryDate) && delivery.status !== 'completed' && (
                      <div className="text-xs text-red-500">지연됨</div>
                    )}
                    {delivery.actualDeliveryDate && (
                      <div className="text-xs text-green-600">실제: {delivery.actualDeliveryDate}</div>
                    )}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div className="w-full bg-gray-200 rounded-full h-2 mb-1">
                      <div 
                        ...
                        style={{ width: `${getProgressPercentage(delivery.totalReceivedQuantity, delivery.totalOrderedQuantity)}%` }}
                      ></div>
                    </div>
                    <div className="text-xs text-gray-600">
                      {getProgressPercentage(delivery.totalReceivedQuantity, delivery.totalOrderedQuantity)}%
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="text-sm text-gray-900">
                      {delivery.productCount}종 / {delivery.totalOrderedQuantity}개
                    </div>
                    <div className="text-xs text-gray-500">
                      입고: {delivery.totalReceivedQuantity}개 / 잔여: {delivery.remainingQuantity}개
                    </div>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    {delivery.deliveryCompany && (
                      <div className="text-sm text-gray-900">{delivery.deliveryCompany}</div>
                    )}
                    {delivery.trackingNumber && (
                      <div className="text-xs text-blue-600">{delivery.trackingNumber}</div>
                    )}
                  </td>
                  <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-semibold">입고 처리 - {selectedDelivery.orderNumber}</h2>
              <button
            ...
            <div>
              <h3 className="text-lg font-medium text-gray-900 mb-3">입고 상품 목록</h3>
              <div className="overflow-x-auto">
                <table className="w-full border border-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-left text-sm font-medium text-gray-500">기입고</th>
                      <th className="px-4 py-2 text-left text-sm font-medium text-gray-500">잔여수량</th>
                      <th className="px-4 py-2 text-left text-sm font-medium text-gray-500">입고수량</th>
                      <th className="px-4 py-2 text-left text-sm font-medium text-gray-500">로트번호</th>
                      <th className="px-4 py-2 text-left text-sm font-medium text-gray-500">유통기한</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200">
                    {receiveItems.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td className="px-4 py-2 text-sm text-gray-600">{item.orderedQuantity - item.remainingQuantity}개</td>
                        <td className="px-4 py-2 text-sm text-gray-900">{item.remainingQuantity}개</td>
                        <td className="px-4 py-2">
                          <input
                            type="number"
                            value={item.receivedQuantity}
                            onChange={(e) => handleUpdateReceiveQuantity(item.id, parseInt(e.target.value) || 0)}
            ...
            <div className="flex justify-between items-center mt-6">
              <div className="text-sm text-gray-600">
                총 입고 예정: {receiveItems.reduce((sum, item) => sum + item.receivedQuantity, 0)}개
              </div>
              <div className="flex space-x-2">
                <button
                  onClick={() => setIsReceiveModalOpen(false)}
      ...
      {isDetailModalOpen && selectedDelivery && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-5xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-semibold">입고예정 상세 - {selectedDelivery.orderNumber}</h2>
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div className="space-y-4">
                <div>
                  ...
                  <div className="text-sm text-gray-900">{selectedDelivery.supplierName}</div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">예정일</label>
                  <div className="text-sm text-gray-900">{selectedDelivery.expectedDeliveryDate}</div>
                </div>
                <div>
                ...
                {selectedDelivery.memo && (
                  <div>
                    ...
                    <div className="text-sm text-gray-900">{selectedDelivery.memo}</div>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">배송회사</label>
                  <div className="text-sm text-gray-900">{selectedDelivery.deliveryCompany || '-'}</div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">송장번호</label>
                  <div className="text-sm text-gray-900">{selectedDelivery.trackingNumber || '-'}</div>
                </div>
                <div>
                  ...
                  <div className="flex items-center space-x-2">
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div 
                        className="bg-blue-600 h-2 rounded-full" 
                        style={{ width: `${getProgressPercentage(selectedDelivery.totalReceivedQuantity, selectedDelivery.totalOrderedQuantity)}%` }}
                      ></div>
                    </div>
                    <span className="text-sm text-gray-600">
                      {getProgressPercentage(selectedDelivery.totalReceivedQuantity, selectedDelivery.totalOrderedQuantity)}%
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div>
              <h3 className="text-lg font-medium text-gray-900 mb-3">상품별 입고 현황</h3>
              <div className="overflow-x-auto">
                <table className="w-full border border-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-left text-sm font-medium text-gray-500">입고수량</th>
                      <th className="px-4 py-2 text-left text-sm font-medium text-gray-500">잔여수량</th>
                      <th className="px-4 py-2 text-left text-sm font-medium text-gray-500">진행률</th>
                      <th className="px-4 py-2 text-left text-sm font-medium text-gray-500">입고일</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200">
                    {mockExpectedDeliveryItems
                      .filter(item => item.expectedDeliveryId === selectedDelivery.id)
                      .map((item) => (
                        <tr key={item.id}>
                          <td className="px-4 py-2 text-sm text-gray-900">{item.productName}</td>
                          <td className="px-4 py-2 text-sm text-gray-500">{item.sku}</td>
                          <td className="px-4 py-2 text-sm text-gray-900">{item.orderedQuantity}개</td>
                          <td className="px-4 py-2 text-sm text-gray-900">{item.receivedQuantity}개</td>
                          <td className="px-4 py-2 text-sm text-gray-900">{item.remainingQuantity}개</td>
                          <td className="px-4 py-2">
                            <div className="w-16 bg-gray-200 rounded-full h-2">
                              <div 
                                className="bg-blue-600 h-2 rounded-full" 
                                style={{ width: `${getProgressPercentage(item.receivedQuantity, item.orderedQuantity)}%` }}
                              ></div>
                            </div>
                          </td>
