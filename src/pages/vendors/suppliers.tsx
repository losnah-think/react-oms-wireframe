...
const SuppliersPage: React.FC = () => {
  ...
  React.useEffect(()=>{
    ...
    return ()=>{ window.removeEventListener('supplier:updated', onUpdated); window.removeEventListener('supplier:deleted', onUpdated) }
  }, [load])

  const onAdd = async () => {
    const created = await api.createSupplier({ name: '새 공급처', contact: {} })
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <SuppliersToolbar total={items.length} q={q} onChangeQ={setQ} onAdd={onAdd} onExport={()=>{ alert('CSV (demo)') }} />
      {loading ? <div>로딩중...</div> : <SuppliersTable items={items} onOpen={onOpen} onDelete={onDelete} />}
      <SupplierDrawer open={drawerOpen} supplier={selectedSupplier} onClose={() => setDrawerOpen(false)} onSave={onSave} />
    </div>
  )
}
