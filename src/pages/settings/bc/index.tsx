          ...
          <div className="col-span-3 bg-white p-4 rounded shadow">
            ...
            <div className="mt-4">
              ...
              <div className="space-y-2">
                {formats.map(f => (
                  <div key={f.id} className={`p-2 rounded ${selectedFormatId === f.id ? 'bg-primary-50' : 'bg-gray-50'}`}>
                    <div className="flex items-center justify-between">
                      <div className="text-sm">{f.name}</div>
                      <div className="flex items-center space-x-2">
                        <button onClick={() => setSelectedFormatId(f.id)} className="text-xs px-2 py-1 border rounded">Select</button>
                        <button onClick={() => { const next = generateNextForFormat(f.id); if (selectedId) update(selectedId, { value: next }) }} className="text-xs px-2 py-1 border rounded">Apply</button>
                        <button onClick={() => removeFormat(f.id)} className="text-xs px-2 py-1 text-red-600">Del</button>
                      </div>
                    </div>
                    <div className="text-xs text-gray-500 mt-1">{f.pattern}</div>
                  </div>
                ))}
                <button onClick={() => { const id = createFormat(); setSelectedFormatId(id) }} className="text-sm text-primary-600">New format</button>
              </div>
              ...
              {selectedFormatId && (() => {
                ...
                return (
                  <div className="mt-4 p-3 border rounded bg-white">
                    <div className="text-sm font-medium mb-2">Edit Format</div>
                    <label className="block text-xs text-gray-500">Name</label>
                    <input className="w-full border px-2 py-1 rounded mb-2" value={fmt.name} onChange={e => updateFormat(fmt.id, { name: e.target.value })} />
                    <label className="block text-xs text-gray-500">Pattern</label>
                    <input className="w-full border px-2 py-1 rounded mb-2" value={fmt.pattern} onChange={e => updateFormat(fmt.id, { pattern: e.target.value })} />
                    <label className="block text-xs text-gray-500">Next Sequence</label>
                    <div className="flex items-center space-x-2 mb-2">
                      <input type="number" className="w-24 border px-2 py-1 rounded" value={fmt.seq ?? 1} onChange={e => updateFormat(fmt.id, { seq: Number(e.target.value) || 1 })} />
                      <button onClick={() => updateFormat(fmt.id, { seq: 1 })} className="text-xs px-2 py-1 border rounded">Reset</button>
                      <button onClick={() => { const next = generateNextForFormat(fmt.id); alert('Preview: ' + next) }} className="text-xs px-2 py-1 border rounded">Preview Next</button>
                    </div>
                  </div>
                )
              })()}
            ...
            <div className="mt-4">
              <TemplateList
                ...
                onDuplicate={(id) => { const nid = duplicate(id); setSelectedId(nid) }}
              />
            </div>
          </div>
...
function PreviewModal({ open, onClose, template }: { open: boolean, onClose: () => void, template?: BarcodeTemplate | null }) {
  ...
  const renderContent = (tmpl: string) => {
    ...
    return tmpl.replace(/\{barcode\}/g, template.value || '')
      .replace(/\{qrcode\}/g, template.value || '')
      .replace(/\{seq\}/g, '1')
      .replace(/\{name\}/g, product.name || '')
      .replace(/\{supplier\}/g, supplier?.name || String(product.supplier_id || ''))
      ...
      .replace(/\{vendor\}/g, supplier?.name || String(product.supplier_id || ''))
      .replace(/\{option\}/g, (product.variants && product.variants[0] ? product.variants[0].variant_name : ''))
  }
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="col-span-2">
            <div className="p-4 border h-64">
              <div className="mb-2 text-sm text-gray-700">Barcode ({template.barcodeType || 'code128'})</div>
              <div className="w-full h-40 bg-white flex items-center justify-center barcode-preview-root">
                  <div className="flex items-center space-x-4">
                    <div><BarcodePreview template={template} /></div>
                    <div>{template.content && template.content.includes('{qrcode}') ? <QRImage value={template.value || ''} size={96} /> : null}</div>
                  </div>
                </div>
            </div>
          </div>
          <div>
            <div className="p-4 border h-64 overflow-auto text-sm">
              ...
              <pre className="whitespace-pre-wrap">{renderContent(String(template.content || ''))}</pre>
            </div>
          </div>
        </div>
      </div>
...
function useLocalTemplates() {
  ...
  const [templates, setTemplates] = useState<BarcodeTemplate[]>(() => {
    ...
    return [
      { id: 't-default', name: 'Default - 1up', value: '012345678901', barcodeType: 'code128', scale: 3, height: 10, includetext: true, quantity: 1 },
      { id: 't-small', name: 'Small Label', value: 'ABC-0001', barcodeType: 'code128', scale: 2, height: 8, includetext: false, quantity: 1 }
    ]
  })
  ...
  const create = () => {
    ...
    const t: BarcodeTemplate = { id, name: 'New Template', value: '', barcodeType: 'code128', scale: 3, height: 10, includetext: true, quantity: 1 }
    setTemplates((s: BarcodeTemplate[]) => [t, ...s])
    return id
  }

  const update = (id: string, patch: Partial<BarcodeTemplate>) => {
    setTemplates((s: BarcodeTemplate[]) => s.map((t: BarcodeTemplate) => t.id === id ? { ...t, ...patch } : t))
  }

  const remove = (id: string) => {
    setTemplates((s: BarcodeTemplate[]) => s.filter((t: BarcodeTemplate) => t.id !== id))
  }
  ...
  const duplicate = (id: string) => {
    ...
    setTemplates((s: BarcodeTemplate[]) => {
      ...
      const dup: BarcodeTemplate = { ...found, id: newId, name: found.name + ' (copy)' }
      return [dup, ...s]
    })
    return newId
  }
...
function useCodeFormats() {
  ...
  const [formats, setFormats] = React.useState<CodeFormat[]>(() => {
    ...
    return [{ id: 'fmt-default', name: 'Default', pattern: '{DATE:yyyyMMdd}-{SEQ:4}', seq: 1 }]
  })

  React.useEffect(() => {
    try { if (typeof window === 'undefined') return; localStorage.setItem(KEY, JSON.stringify(formats)) } catch (e) {}
  }, [formats])

  const createFormat = () => {
    const id = 'fmt-' + Math.random().toString(36).slice(2,9)
    const f: CodeFormat = { id, name: 'New format', pattern: '{DATE:yyyyMMdd}-{SEQ:4}', seq: 1 }
    setFormats(s => [f, ...s])
    return id
  }

  const updateFormat = (id: string, patch: Partial<CodeFormat>) => setFormats(s => s.map(f => f.id === id ? { ...f, ...patch } : f))
  const removeFormat = (id: string) => setFormats(s => s.filter(f => f.id !== id))
  ...
  const generateNextForFormat = (id: string, sku?: string) => {
    let out = ''
    setFormats(s => s.map(f => {
      if (f.id !== id) return f
      const pattern = f.pattern || ''
      let result = pattern.replace(/\{DATE:([^}]+)\}/g, (_, p) => fmtDate(new Date(), p))
  ...
  return { formats, createFormat, updateFormat, removeFormat, generateNextForFormat }
}
...
function handleGeneratePdf() {
  ...
  const el = document.querySelector('.barcode-preview-root') as HTMLElement | null
  if (!el) {
    window.print()
    return
  }
