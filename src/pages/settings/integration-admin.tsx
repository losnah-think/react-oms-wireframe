...
export default function IntegrationAdminPage({}) {
  return (
    <div className="p-6">
      ...
      <p>
        관리자 전용 페이지입니다. 여기에서 clientSecret을 안전하게 저장할 수
        있습니다.
      </p>
      <div className="mt-4">
        <form
          ...
          onSubmit={async (e) => {
            e.preventDefault();
            const shopId = (
              document.getElementById("shopId") as HTMLInputElement
            ).value;
            const secret = (
              document.getElementById("secret") as HTMLInputElement
            ).value;
            const resp = await fetch("/api/integrations/cafe24/set-secret", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ shopId, clientSecret: secret }),
            });
