...
export default function BasicMetadataSettings() {
  ...
  const addSeason = () => {
    ...
    if (!val) return;
    setSeasons((s) => [
      { id: makeId("s"), name: val, yearId: newSeasonYear || undefined },
      ...s,
    ]);
    setNewSeason("");
    setNewSeasonYear("");
    ...
    <Container maxWidth="full" centered={false} padding="md">
      ...
      <div>
        <div className="flex gap-2 mb-4">
          <button
            className={`px-3 py-1 rounded ${activeTab === "brands" ? "bg-primary-600 text-white" : "bg-gray-100"}`}
            onClick={() => setActiveTab("brands")}
          >
            브랜드
          </button>
          <button
            className={`px-3 py-1 rounded ${activeTab === "years" ? "bg-primary-600 text-white" : "bg-gray-100"}`}
            onClick={() => setActiveTab("years")}
          >
            연도
          </button>
          <button
            className={`px-3 py-1 rounded ${activeTab === "seasons" ? "bg-primary-600 text-white" : "bg-gray-100"}`}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-sm text-gray-500">
                      {s.yearId
                        ? (years.find((y) => y.id === s.yearId)?.name ??
                          "삭제된 연도")
                        : "연도 미지정"}
                    </div>
                  </div>
