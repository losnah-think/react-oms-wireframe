...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await signIn('credentials', { redirect: false, json: true, email, password, callbackUrl: callback })
      if ((result as any)?.ok) {
          ...
          <div className="backdrop-blur-sm bg-white/60 border border-white/40 rounded-2xl shadow-xl p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input id="email" name="email" aria-invalid={!!error} placeholder="email" className="mt-1 block w-full rounded-md border-gray-200 px-3 py-2 shadow-sm" />
              </div>
              <div>
                <label htmlFor="password" className="block text-sm font-medium text-slate-700">Password</label>
                <input id="password" name="password" aria-invalid={!!error} placeholder="password" type="password" className="mt-1 block w-full rounded-md border-gray-200 px-3 py-2 shadow-sm" />
              </div>
              <div className="flex items-center justify-between">
                ...
                <div>
                  <button aria-label="Sign in" disabled={loading} className="inline-flex items-center px-4 py-2 bg-primary-600 text-white rounded-md shadow">{loading ? '로그인 중...' : '로그인'}</button>
                </div>
              </div>
            </form>
          </div>
