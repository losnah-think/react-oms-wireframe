      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-600">상품 분류를 추가, 편집 및 삭제합니다. 트리 구조를 지원합니다.</p>
          </div>
          <div>
            <button className="px-3 py-2 bg-primary-600 text-white rounded" onClick={openAddRoot}>루트 분류 추가</button>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <div className="space-y-3 pl-8">
            {tree.length === 0 && <div className="text-sm text-gray-500">분류가 없습니다.</div>}
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className={`bg-white rounded-lg shadow-lg w-full ${modalClass}`} style={{ maxHeight: '80vh', overflow: 'auto' }}>
            <div className="flex items-center justify-between p-4 border-b">
              <div>
                <h3 className="text-lg font-semibold">{editing && editing.node ? '분류 편집' : '분류 추가'}</h3>
            ...
            <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
                {!editing?.node ? (
                  <>
                    <label className="block text-sm text-gray-700">부모 분류 선택 (선택)</label>
                    <div className="mt-2">
                      <HierarchicalSelect
                  ...
                  <div className="text-sm text-gray-600">편집 모드입니다. 위치 변경은 분류 삭제 후 재생성으로 처리하세요.</div>
                )}
              </div>
            </div>
