  ...
  return (
    <Container maxWidth="md">
      <Card className="p-4">
        ...
        {orderId && !order && <div>해당 주문을 찾을 수 없습니다: {orderId}</div>}
        {order && (
          <div className="space-y-3">
            <div><strong>주문번호:</strong> {order.orderId}</div>
            <div><strong>주문코드:</strong> {order.orderCode}</div>
            <div><strong>회원:</strong> {order.memberId}</div>
            <div><strong>상태:</strong> {order.orderStatus}</div>
            <div><strong>주문일:</strong> {order.orderDate}</div>
            <div><strong>수령인:</strong> {order.shipping.receiverName}</div>
            <div><strong>결제금액:</strong> {order.payment.amount.toLocaleString()}원</div>
            <div>
              <strong>아이템:</strong>
              <ul className="list-disc ml-6">
                {order.items.map((it: any) => (
                  <li key={it.productNo}>{it.productName} x {it.quantity} ({it.price}원)</li>
                ))}
              </ul>
            </div>
          </div>
