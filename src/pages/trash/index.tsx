...
export default function TrashPage() {
  ...
  const refresh = () => {
    try { const raw = localStorage.getItem('trashed_products_v1'); setTrashed(raw ? JSON.parse(raw) : []) } catch (e) { setTrashed([]) }
  }
  ...
  const restoreSelected = () => {
    ...
    if (ids.length === 0) { alert('복원할 항목을 선택하세요.'); return }
