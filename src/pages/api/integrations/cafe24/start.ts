...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!shopId || !clientId) return res.status(400).json({ error: 'shopId and clientId required' })

  // determine origin to build redirectUri
  const origin = (req.headers.origin as string) || `${req.headers['x-forwarded-proto'] || 'https'}://${req.headers.host}`
