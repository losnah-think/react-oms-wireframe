...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const redirectUri = shop?.credentials?.redirectUri || process.env.CAFE24_REDIRECT_URI
  if (!shop || !refreshToken) return res.status(400).json({ error: 'no refresh token' })
  if (!clientId || !clientSecret || !redirectUri) return res.status(400).json({ error: 'missing client credentials' })
