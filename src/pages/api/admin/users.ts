...
export default async function handler(
  ...
  if (req.method === "POST") {
    ...
    if (
      ...
      req.headers["x-dev-user"] === "__dev__"
    ) {
      try {
        ...
        return res
          .status(200)
          .json({ postStatus: resp.status, ok: resp.ok, text, parsed });
      } catch (err) {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          fs.appendFileSync(
            "debug/admin-users.log",
            JSON.stringify({
              ts: Date.now(),
              event: "REQUEST",
              url: `${sbUrl}/rest/v1/users`,
              body,
            }) + "\n",
          );
        } catch (err) {
        ...
        if (!resp.ok)
          throw new Error(
            "Supabase REST insert failed: " + resp.status + " " + text,
          );
        let rows: any;
        try {
        ...
        try {
          const getResp = await nodeFetch(
            `${sbUrl.replace(/\/$/, "")}/rest/v1/users?email=eq.${encodeURIComponent(email)}&select=*`,
            {
              method: "GET",
              headers: {
                apikey: sbKey,
          ...
          try {
            ...
            try {
              for (const [k, v] of (getResp.headers as any).entries())
                getHeadersObj[k] = v;
            } catch (_) {}
            try {
              await fs.promises.mkdir("debug", { recursive: true });
          ...
          if (getResp.ok) {
            let getRows: any;
            try {
              getRows = JSON.parse(getText);
            } catch (err) {
              getRows = getText;
            }
            if (Array.isArray(getRows) && getRows[0])
              return res.status(201).json(getRows[0]);
        ...
        return res
          ...
          .json({
            created: true,
            email,
            raw: rows && rows[0] ? rows[0] : rows,
          });
      } catch (e2) {
        return res
          .status(500)
          .json({ error: "failed to create user", detail: String(e2) });
      }
    }
  }
