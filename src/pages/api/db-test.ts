...
if (!process.env.DATABASE_URL) {
  ...
  console.warn('src/pages/api/db-test: DATABASE_URL is not set')
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  try {
    ...
    const r1 = await query(`SELECT current_database() as db, inet_server_addr() as host, version() as version`)
