...
const ProductsListPage: React.FC<ProductsListPageProps> = ({ onNavigate }) => {
  ...
  const getVariantStockStatus = (product: any) => {
    if (!product.variants || product.variants.length === 0) {
      ...
      if (stock > 50)
        return { status: "충분", count: 1, color: "text-green-600" };
      if (stock > 10)
        return { status: "부족", count: 1, color: "text-yellow-600" };
      return { status: "위험", count: 1, color: "text-red-600" };
    }

    const sufficient = product.variants.filter((v: any) => v.stock > 50).length;
    const insufficient = product.variants.filter(
      (v: any) => v.stock > 10 && v.stock <= 50,
    ).length;
                                    ...
                                    <div className="text-xs mt-1 space-y-1">
                                      {stockStatus.details.sufficient > 0 && (
                                        <div className="text-green-600">
                                          충분: {stockStatus.details.sufficient}
                                        </div>
                                      )}
                                      {stockStatus.details.insufficient > 0 && (
                                        <div className="text-yellow-600">
                                          부족:{" "}
                                          {stockStatus.details.insufficient}
                                        </div>
                                      )}
                                      {stockStatus.details.dangerous > 0 && (
                                        <div className="text-red-600">
                                          위험: {stockStatus.details.dangerous}
                                        </div>
                                      )}
                                    </div>
                                  )}
                    ...
                    <td className="px-6 py-6 whitespace-nowrap">
                      <div className="flex justify-center items-center space-x-2">
                        <Button
                          ...
                          size="small"
                          onClick={() =>
                            onNavigate?.("products-edit", String(product.id))
                          }
                                  ...
                                  >
                                    ...
                                    <div className="text-right space-y-1">
                                      ...
                                      <div className="font-medium text-blue-600">
                                        {formatPrice(variant.supplyPrice)}
                                      </div>
                                      <div className="text-sm text-gray-500">
                                        판매가
                                      </div>
