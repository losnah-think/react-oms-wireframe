...
export default function ProductsTrashPage() {
  ...
  if (ids.length === 0) { toast?.push('복원할 항목을 선택하세요.', 'info'); return }
  ...
  toast?.push(`${ids.length}개 항목을 복원했습니다.`, 'success')
    } catch (e) { console.error(e); alert('복원 중 오류') }
  }

  const deleteSelectedPermanently = () => {
    const ids = Object.keys(selected).filter(k => selected[k])
  if (ids.length === 0) { toast?.push('삭제할 항목을 선택하세요.', 'info'); return }
    ...
    <Container maxWidth="full" padding="lg" className="bg-gray-50 min-h-screen">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={restoreSelected}>복원</Button>
          <Button variant="danger" onClick={deleteSelectedPermanently}>선택 항목 완전 삭제</Button>
          <Button variant="ghost" onClick={emptyTrash}>비우기</Button>
        </div>
      </div>

      <Card padding="lg">
        {trashed.length === 0 ? (
          <div className="p-8 text-center text-gray-600">휴지통이 비어 있습니다.</div>
        ) : (
          <table className="min-w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                <th className="px-4 py-2"><input type="checkbox" onChange={(e) => { const checked = e.target.checked; const next: Record<string, boolean> = {}; if (checked) trashed.forEach(t => next[String(t.id)] = true); setSelected(next) }} /></th>
            ...
            <tbody>
              {trashed.map((t) => (
                <tr key={t.id} className="border-t hover:bg-gray-50">
                  <td className="px-4 py-3"><input type="checkbox" checked={!!selected[String(t.id)]} onChange={() => toggleSelect(String(t.id))} /></td>
                  <td className="px-4 py-3">{t.name}</td>
                  <td className="px-4 py-3">{t.code || '-'}</td>
                  <td className="px-4 py-3">{t.deleted_at || '-'}</td>
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      <Button variant="outline" onClick={() => { setSelected({}); try { const remaining = trashed.filter(x => x.id !== t.id); const rawP = localStorage.getItem('products_local_v1'); const existingProducts = rawP ? JSON.parse(rawP) : []; const existingIds = new Set(existingProducts.map((p:any)=>String(p.id))); if (!existingIds.has(String(t.id))) existingProducts.push(t); localStorage.setItem('products_local_v1', JSON.stringify(existingProducts)); localStorage.setItem('trashed_products_v1', JSON.stringify(remaining)); try{window.dispatchEvent(new CustomEvent('products:updated'))}catch(e){} try{window.dispatchEvent(new CustomEvent('trashed:updated'))}catch(e){} setTrashed(remaining); toast?.push('복원되었습니다.', 'success'); } catch (e) { toast?.push('오류', 'error') } }}>복원</Button>
                      <Button variant="danger" onClick={() => { if (!confirm('영구 삭제하시겠습니까?')) return; try { const remaining = trashed.filter(x => x.id !== t.id); localStorage.setItem('trashed_products_v1', JSON.stringify(remaining)); try { window.dispatchEvent(new CustomEvent('trashed:updated')) } catch(e){} setTrashed(remaining); toast?.push('영구 삭제되었습니다.', 'success'); } catch (e) { toast?.push('오류', 'error') } }}>영구 삭제</Button>
                    </div>
                  </td>
                </tr>
              ))}
