...
export default function ProductImportPage(): JSX.Element {
  ...
  const [autoRegisterCategory, setAutoRegisterCategory] =
  ...
  const fetchCafe24Products = async () => {
    ...
    try {
      ...
      setProducts([
        {
          product_no: 1001,
          product_name: "샘플 상품 A (mock)",
          product_code: "A-MOCK-1001",
          price: 12000,
          stock: 10,
        },
        {
          product_no: 1002,
          product_name: "샘플 상품 B (mock)",
          product_code: "B-MOCK-1002",
          price: 34000,
          stock: 5,
        },
      ]);
        ...
        <div className="flex gap-2">
          {["cafe24", "makeshop", "wisa", "godomall", "smartstore"].map((p) => (
            <button
              ...
              className={`px-3 py-1 border ${platform === p ? "bg-white border-blue-400" : "bg-gray-50"}`}
            >
              {p}
            </button>
          ))}
      ...
      <Card>
        ...
        <div className="mt-4 grid grid-cols-1 gap-4">
          <div className="text-sm text-gray-700">
            선택된 플랫폼: <strong>{platform}</strong>
          </div>
          ...
          {(() => {
              ...
              return (
                <div>
                  <div className="p-3 bg-red-50 border border-red-100 text-sm text-red-700">
                    메이크샵 전용 주의사항: 카테고리, 판매자 정보가 정확해야
                    상품 등록이 성공합니다.
                  </div>
                  <div className="mt-4 bg-white border">
                    <Row label="판매처 선택">
                      <Input
                        value={sellerSelect}
                        onChange={(e: any) => setSellerSelect(e.target.value)}
                        placeholder="판매처 선택"
                      />
                    </Row>
                      ...
                      <div className="flex gap-4 items-center">
                        <label className="inline-flex items-center">
                          <input
                            type="radio"
                            name="priceEdit"
                            checked={priceEditMode === "수정함"}
                            onChange={() => setPriceEditMode("수정함")}
                          />{" "}
                          <span className="ml-2">수정함</span>
                        </label>
                        <label className="inline-flex items-center">
                          <input
                            type="radio"
                            name="priceEdit"
                            checked={priceEditMode === "수정안함"}
                            onChange={() => setPriceEditMode("수정안함")}
                          />{" "}
                          <span className="ml-2">수정안함</span>
                        </label>
                      </div>
                    </Row>
                    ...
                    <Row label="상품분류 등록여부">
                      <div>
                        <input
                          type="checkbox"
                          checked={autoRegisterCategory}
                          onChange={(e) =>
                            setAutoRegisterCategory(e.target.checked)
                          }
                        />{" "}
                        체크시 상품분류가 자동으로 등록됩니다
                      </div>
                    </Row>

                    <div className="p-3 text-sm text-orange-600">
                      옵션 적용, 카테고리 자동등록 등 MakeShop 업로드 규칙을
                      확인하세요.
                    </div>
                  </div>
                </div>
            ...
            if (platform === "cafe24") {
              return (
                <div>
                  <div className="p-3 bg-yellow-50 border border-yellow-100 text-sm text-yellow-700">
                    Cafe24 연동: Mall ID 및 인증 필요
                  </div>
                  <div className="mt-4 bg-white border">
                    <Row label="판매처">
                      <Input
                        value={mallId}
                        onChange={(e: any) => setMallId(e.target.value)}
                        placeholder="Mall ID"
                      />
                    </Row>
                    ...
                    <Row label="다운로드 상품 조건">
                      <select
                        value={priceEditMode}
                        onChange={(e: any) => setPriceEditMode(e.target.value)}
                      >
                        <option>등록일</option>
                        <option>수정일</option>
                      </select>
            ...
            if (platform === "wisa") {
              return (
                <div>
                  <div className="p-3 bg-gray-50 border border-gray-100 text-sm text-gray-700">
                    Wisa 상품 자동등록 설정 및 사용법
                  </div>
                  <div className="mt-4 bg-white border">
                    ...
                    <Row label="상품코드">
                      <div className="flex gap-2 items-center">
                        <Input placeholder="상품코드 형식" />
                        <div className="text-sm text-gray-500">예: SKU123</div>
                      </div>
                    </Row>
                    <div className="p-3 text-sm text-gray-600">
                      아래 표 형식으로 필수 필드를 확인하세요.
                    </div>
                  </div>
                </div>
              );
            ...
            if (platform === "godomall") {
              return (
                <div>
                  <div className="p-3 bg-gray-50 border border-gray-100 text-sm text-gray-700">
                    고도몰 API 상품등록 — 판매가격, 제조사, 대표이미지 등 필수
                    항목을 확인하세요.
                  </div>
                  <div className="mt-4 bg-white border">
                    <Row label="다운로드 상품 조건">
            ...
            return (
              <div>
                <div className="p-3 bg-gray-50 border border-gray-100 text-sm text-gray-700">
                  SmartStore 상품 등록 — 전시관리/상품API 설정을 확인하세요.
                </div>
                <div className="mt-4 bg-white border">
                  <Row label="판매처">
                  ...
                  <Row label="옵션명 적용여부">
                    <div>
                      <input type="checkbox" /> 체크시 옵션명이 저장됩니다
                    </div>
                  </Row>
          ...
          <div>
            ...
            <div className="p-3 bg-red-50 border border-red-100 text-sm text-red-700">
              상품 등록 API 사용을 위해서는 전시관리/스토어 관리에서 상품API가
              설정되어 있어야 합니다. 필수 조건: '스마트스토어' 연동 정보가
              정확해야 합니다.
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <input
                  id="applyOption"
                  type="checkbox"
                  checked={applyOptionName}
                  onChange={(e) => setApplyOptionName(e.target.checked)}
                />
                <label htmlFor="applyOption" className="text-sm">
                  체크 시 옵션명이 "색상: 빨강, 사이즈: M" 으로 저장됩니다
                </label>
              </div>
            </div>
            ...
            <div>
              ...
              <select
                value={productVisibility}
                onChange={(e: any) => setProductVisibility(e.target.value)}
                className="mt-1"
              >
                <option>전체</option>
                <option>진열함</option>
              ...
              <div className="flex gap-4 mt-2">
                <label className="inline-flex items-center">
                  <input
                    type="radio"
                    name="codeMode"
                    checked={productCodeMode === "상품코드"}
                    onChange={() => setProductCodeMode("상품코드")}
                  />{" "}
                ...
                <label className="inline-flex items-center">
                  <input
                    type="radio"
                    name="codeMode"
                    checked={productCodeMode === "상품코드+서브코드"}
                    onChange={() => setProductCodeMode("상품코드+서브코드")}
                  />{" "}
                  <span className="ml-2">상품코드+서브코드</span>
                </label>
              </div>
            </div>
        ...
        <div className="mt-4 flex items-center gap-2">
          <Button onClick={fetchCafe24Products} disabled={isLoading}>
            조회하기
          </Button>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm min-w-0">
                ...
                <tbody>
                  {products.map((p: any) => (
                    <tr
                      key={p.product_no ?? p.id ?? Math.random()}
