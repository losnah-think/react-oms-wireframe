...
const getBarcodeMeta = (val?: string | null) => {
  if (!val) return { type: '—' as const, validChecksum: undefined as boolean | undefined }
...
const toneClasses: Record<'neutral' | 'success' | 'warning' | 'error' | 'info', string> = {
...
const Chip: React.FC<{ tone?: keyof typeof toneClasses; className?: string; children: React.ReactNode }> = ({ tone = 'neutral', className = '', children }) => (
  <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${toneClasses[tone]} ${className}`}>{children}</span>
)
...
const saveLog = (entry: any) => {
  try {
    ...
    arr.unshift(entry)
    localStorage.setItem('barcodes_logs_v1', JSON.stringify(arr.slice(0, 1000)))
  } catch (e) { /* ignore */ }
}
...
const ProductBarcodesPage: React.FC = () => {
  ...
  const [statusFilter, setStatusFilter] = React.useState<'all' | 'missing' | 'error' | 'duplicate' | 'normal'>('all')
  ...
  const stats = React.useMemo(() => {
    let missing = 0, error = 0, normal = 0, duplicate = 0
    products.forEach(p => {
  ...
  React.useEffect(() => {
    const list = products.filter(p => {
      ...
      return (p.title || '').toLowerCase().includes(q) || (p.sku || '').toLowerCase().includes(q) || (p.barcode || '').toLowerCase().includes(q)
    })
    const list2 = list.filter(p => {
      const st = getBarcodeState(p.barcode)
      const dup = p.barcode ? (barcodeCounts[p.barcode] > 1) : false
      switch (statusFilter) {
        case 'missing': return !p.barcode
        case 'error': return st === '오류'
        case 'duplicate': return dup
        case 'normal': return st === '정상' && !dup
        default: return true
      }
    })
    setFiltered(list2)
  ...
  const doSelectAll = () => {
    ...
    filtered.forEach(p => { newAll[p.id] = true })
    setSelected(newAll)
    setSelectAll(true)
  }
  ...
  const exportCsv = () => {
    const rows = filtered.map(p => ({ id: p.id, title: p.title, sku: p.sku, barcode: p.barcode || '' }))
  ...
  const autoIssue = async (ids?: string[]) => {
    const targets = ids ? products.filter(p => ids.includes(p.id)) : products.filter(p => !p.barcode)
    ...
    for (const t of targets) {
      ...
      if (idx >= 0) {
        ...
        count++
        saveLog({ ts: new Date().toISOString(), user: 'system', action: 'auto-issue', id: t.id, value: code })
      }
      await new Promise(res => setTimeout(res, 50))
    }
    setProducts(updated)
    setFiltered(prev => prev.map(p => ({ ...updated.find(u => u.id === p.id) || p })))
  ...
  const apiSaveBarcode = async (id: string, barcode: string | null) => {
    ...
    try {
      ...
      if (idx >= 0) { arr[idx].barcode = barcode }
  ...
  const apiSync = async () => {
    ...
    const raw = localStorage.getItem('products_local_v1')
    const arr = raw ? JSON.parse(raw) : []
    const norm = Array.isArray(arr) ? arr.map((p: any, i: number) => ({ id: String(p.id ?? p._id ?? p.sku ?? i), title: p.title ?? '', barcode: p.barcode ?? null, sku: p.sku ?? '' })) : []
    setProducts(norm)
    setFiltered(norm)
  ...
  const saveEdit = (id: string) => {
    ...
    if (/^\d{13}$/.test(raw) && !isValidEan13(raw)) { alert('EAN-13 체크섬 불일치') ; return }
    const updated = products.map(p => p.id === id ? { ...p, barcode: manualValue || null } : p)
    setProducts(updated)
    saveLog({ ts: new Date().toISOString(), user: 'operator', action: 'manual-edit', id, value: manualValue })
    setShowEditId(null)
  }
  ...
  const autofillSelected = async () => {
    ...
    const updated = products.map(p => {
      ...
      if (/^\d{12}$/.test(raw)) {
        ...
        saveLog({ ts: new Date().toISOString(), user: 'operator', action: 'autofill', id: p.id, value: newCode })
  ...
  const invalidateSelected = async () => {
    const ids = Object.keys(selected).filter(k => selected[k])
    if (ids.length === 0) { alert('선택된 항목이 없습니다'); return }
    for (const id of ids) { await apiSaveBarcode(id, null); saveLog({ ts: new Date().toISOString(), user: 'operator', action: 'invalidate', id }) }
    await apiSync()
    detectConflicts()
    clearSelection()
    alert('선택 항목 무효화 완료')
  ...
  const [printOptions, setPrintOptions] = React.useState({ size: '50x30', copies: 1 })
  ...
  <Container maxWidth="full" centered={false} padding="md">
      <Grid container gutter={[8, 8]} className="mb-4">
        ...
        <GridCol span={2} className="flex justify-end items-start">
          <Button variant="outline" size="small" className="border-blue-400 text-blue-600">중복 감지</Button>
        </GridCol>

        {/* Toolbar row 1 (primary) */}
        <GridCol span={16} className="flex items-center gap-2 mt-3">
          <div className="flex gap-2">
            <Button variant="primary" size="small" onClick={() => autoIssue()}><Icon name="plus" size={12} /> 자동 발급</Button>
            <Button size="small" onClick={printLabels}><Icon name="document" size={12} /> 인쇄</Button>
            <Button variant="outline" size="small" onClick={exportCsv}><Icon name="download" size={12} /> CSV</Button>
            <Button variant="outline" size="small" onClick={apiSync}><Icon name="download" size={12} /> 동기화</Button>
          </div>
        </GridCol>
        ...
        <GridCol span={16} className="flex items-center gap-2">
          <div className="flex gap-2">
            <Button variant="ghost" size="small" onClick={() => doSelectAll()}>전체선택</Button>
            <Button variant="ghost" size="small" onClick={invalidateSelected}>선택 무효화</Button>
            <Button variant="ghost" size="small" onClick={autofillSelected}>체크섬 완성</Button>
            <Button variant="ghost" size="small" onClick={detectConflicts}>중복 감지</Button>
            <Button variant="ghost" size="small" onClick={openLogs}>발급 로그</Button>
          </div>
        </GridCol>
      </Grid>
      ...
      <Card padding="lg" className="mb-4">
        <Grid container gutter={[12, 12]}>
          <GridCol span={12}>
            <input type="text" className="border px-3 py-2 rounded w-full" placeholder="상품명/코드/바코드 검색" value={query} onChange={e => setQuery(e.target.value)} />
          </GridCol>
          <GridCol span={8} className="flex items-center gap-2 flex-wrap">
            <button type="button" onClick={() => setStatusFilter('all')} className={`px-3 py-1 rounded-full text-xs ${statusFilter === 'all' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-700'}`}>전체</button>
            <button type="button" onClick={() => setStatusFilter('missing')} className={`px-3 py-1 rounded-full text-xs ${statusFilter === 'missing' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-700'}`}>미발급 {stats.missing}</button>
            <button type="button" onClick={() => setStatusFilter('error')} className={`px-3 py-1 rounded-full text-xs ${statusFilter === 'error' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-700'}`}>오류 {stats.error}</button>
            <button type="button" onClick={() => setStatusFilter('duplicate')} className={`px-3 py-1 rounded-full text-xs ${statusFilter === 'duplicate' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-700'}`}>중복 {stats.duplicate}</button>
            <button type="button" onClick={() => setStatusFilter('normal')} className={`px-3 py-1 rounded-full text-xs ${statusFilter === 'normal' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-700'}`}>정상 {stats.normal}</button>
          </GridCol>
      ...
      <div className="mb-4">
        <Grid container gutter={[8,8]}>
          <GridCol span={6} className="flex items-center gap-2">
            <Button variant="outline" onClick={() => doSelectAll()}>전체 선택</Button>
            <Button onClick={() => autoIssue(Object.keys(selected).filter(k => selected[k]))}>선택 항목 자동 발급</Button>
            <Button onClick={() => autoIssue()}>필터 대상 일괄 자동 발급</Button>
          </GridCol>
          ...
          <table className="w-full table-fixed text-sm border-collapse min-w-[900px]">
            <thead className="bg-white sticky top-0">
              <tr className="text-left text-gray-600 border-b">
                ...
                <th className="p-2" style={{width:'6%'}}>이미지</th>
                <th className="p-2" style={{width:'34%'}}>상품명</th>
                <th className="p-2 text-center" style={{width:'4%'}}><input type="checkbox" checked={selectAll} onChange={e => e.target.checked ? doSelectAll() : clearSelection()} /></th>
                <th className="p-2 text-center" style={{width:'16%'}}>바코드번호</th>
                <th className="p-2" style={{width:'18%'}}>옵션정보</th>
                <th className="p-2 text-right" style={{width:'6%'}}>원가</th>
                <th className="p-2 text-center" style={{width:'8%'}}>인쇄대기</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((p, idx) => {
                    ...
                    <td className="p-2 text-sm min-w-0">
                      ...
                      <div className="mt-2 flex items-center gap-2 min-w-0">
                        <div className="flex-shrink-0">
                          <Button variant="outline" size="small" onClick={() => {/* PDF 인쇄 */}}>PDF 인쇄</Button>
                        </div>
                        <div className="flex-shrink-0">
                          <Button variant="outline" size="small" onClick={() => openEdit(p.id)}>수정</Button>
                        </div>
                        <div className="flex-shrink-0">
                          <Button variant="outline" size="small" onClick={() => openLogs()}>내역</Button>
                        </div>
                        <div className="text-sm text-red-600 ml-2 truncate">현재고: 0</div>
                      </div>
                    </td>
                    <td className="p-2 text-sm text-right">{(Math.floor(1000 + idx * 500)).toLocaleString()}원</td>
                    <td className="p-2 text-sm text-center min-w-0">
                      <div className="flex items-center gap-2 justify-center min-w-0">
                        <input className="border px-2 py-1 w-24 text-sm flex-shrink-0" placeholder="수량" />
                        <div className="flex-shrink-0">
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={8} className="p-6 text-center text-sm text-gray-500">
                    <div className="max-w-xl mx-auto">조건에 맞는 상품이 없습니다. 필터를 조정하거나 ‘미발급만 보기’를 해제해 보세요.</div>
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {showEditId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center">
          <div className="bg-white p-6 rounded w-[520px]">
            ...
            <p className="text-sm text-gray-600 mb-4">EAN-13: 13자리 숫자, 체크섬 자동 계산/검증</p>
            <div className="flex gap-2 mb-3">
              <input className="border w-full p-2" value={manualValue} onChange={e => setManualValue(e.target.value)} placeholder="바코드값 입력(예: 8801234567890)" />
              <Button variant="outline" onClick={() => setManualValue(autofillChecksumForValue(manualValue))}>체크섬 완성</Button>
            </div>
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setShowEditId(null)}>취소</Button>
              <Button variant="primary" onClick={() => saveEdit(showEditId)}>저장</Button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center">
          <div className="bg-white p-6 rounded w-[800px] max-h-[80vh] overflow-auto">
            ...
            <div className="space-y-2">
              {(JSON.parse(localStorage.getItem('barcodes_logs_v1') || '[]') || []).map((l: any, i: number) => (
                <div key={i} className="border-b py-2 text-sm">
                  <div className="font-medium">{l.action} — {l.id}</div>
                  <div className="text-xs text-gray-500">{l.ts} · {l.user} · {String(l.value || '')}</div>
                </div>
              ))}
            </div>
            <div className="flex justify-end mt-4">
              <Button variant="outline" onClick={() => setLogsOpen(false)}>닫기</Button>
            </div>
          </div>
        </div>
      )}
      ...
      {conflictOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center">
          <div className="bg-white p-6 rounded w-[720px] max-h-[80vh] overflow-auto">
            <h3 className="text-lg font-semibold mb-2">중복 바코드 충돌 목록</h3>
            <div className="text-sm text-gray-600 mb-4">같은 바코드가 여러 SKU에 할당되어 있습니다. 조치하세요.</div>
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-gray-600"><th>바코드</th><th>상품명</th><th>SKU</th><th>액션</th></tr>
              </thead>
              <tbody>
                {conflictList.map((c, i) => (
                  <tr key={i} className="border-t">
                    ...
                    <td className="p-2">
                      <div className="flex gap-2">
                        <Button size="small" variant="outline" onClick={() => { setManualValue(c.barcode); setShowEditId(c.id); setConflictOpen(false); }}>교체</Button>
                        <Button size="small" onClick={async () => { await apiSaveBarcode(c.id, null); detectConflicts(); }}>무효화</Button>
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            <div className="flex justify-end mt-4">
              <Button variant="outline" onClick={() => setConflictOpen(false)}>닫기</Button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center">
          <div className="bg-white p-6 rounded w-[640px] max-h-[80vh] overflow-auto">
            ...
            <div className="mb-3">
              ...
              <select value={printOptions.size} onChange={e => setPrintOptions(prev => ({ ...prev, size: e.target.value }))} className="border px-2 py-1 rounded">
              ...
              <input type="number" value={printOptions.copies} onChange={e => setPrintOptions(prev => ({ ...prev, copies: Math.max(1, Number(e.target.value) || 1) }))} className="border px-2 py-1 rounded w-20" />
            </div>
            ...
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setPrintModalOpen(false)}>닫기</Button>
              <Button onClick={() => { setPrintModalOpen(false); setTimeout(() => window.print(), 400) }}>프린트</Button>
            </div>
          </div>
        </div>
      )}
