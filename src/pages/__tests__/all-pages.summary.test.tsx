...
jest.mock('../../components/layout/Header', () => ({ __esModule: true, default: () => React.createElement('header', null, 'Header Mock') }));
jest.mock('../../components/layout/Sidebar', () => ({ __esModule: true, default: (props: any) => React.createElement('aside', null, 'Sidebar Mock') }));
...
describe('All pages render summary (non-failing)', () => {
  it('attempts to render each page and prints a summary table', () => {
    ...
    const results: Array<{ page: string; status: 'pass' | 'fail'; message?: string }> = [];
      ...
      try {
        ...
        if (typeof Comp === 'function') {
          ...
          try {
            ...
            results.push({ page: rel, status: 'fail', message: (err && err.message) ? err.message : String(err) });
          }
        } else {
