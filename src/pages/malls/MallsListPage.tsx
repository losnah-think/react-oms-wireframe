            ...
            {[
              ...
              <div key={index} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h3 className="text-sm font-medium text-gray-900">{mall.name}</h3>
                    <p className="text-sm text-gray-500">주문 {mall.orders}건</p>
                  </div>
                </div>
                <div className="flex items-center space-x-3">
                  <span className={`px-2 py-1 text-xs rounded ${
                    mall.status === '연동됨' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                  }`}>
                    {mall.status}
                  </span>
                  <button className="px-3 py-1 text-xs bg-gray-100 text-gray-700 rounded">설정</button>
                </div>
              </div>
            ))}
          </div>
