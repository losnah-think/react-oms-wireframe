...
const MallProductsPage: React.FC = () => {
  ...
  const [showEditModal, setShowEditModal] = useState<{ productId: string } | null>(null)
  ...
  const applyOverridesToProduct = (product: MallProduct) => {
    ...
    const found = Object.values(overrides || {}).find((o: any) => o.productId === product.productId)
    return found ? { ...product, ...found } : product
  }

  const filteredProducts = products.filter((product) => {
    const effective = applyOverridesToProduct(product)
    const matchesSearch = effective.name.toLowerCase().includes(searchTerm.toLowerCase()) || effective.mallProductName.toLowerCase().includes(searchTerm.toLowerCase())
  ...
  const handleSyncProduct = (productId: number) => {
    setProducts((prev) => prev.map((p) => (p.id === productId ? { ...p, syncStatus: 'synced', lastSync: new Date().toLocaleString('ko-KR'), mallPrice: p.price, mallStock: p.stock } : p)))
    alert('상품이 동기화되었습니다.')
  }

  const handleSyncAll = () => {
    if (!window.confirm('모든 상품을 동기화하시겠습니까?')) return
    setProducts((prev) => prev.map((p) => ({ ...p, syncStatus: 'synced', lastSync: new Date().toLocaleString('ko-KR'), mallPrice: p.price, mallStock: p.stock })))
      ...
      <div className="p-6">
        ...
        <div className="bg-white border rounded-lg p-4 mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            {selectedMall && (
              <div className="flex items-center gap-2">
                <button onClick={handleSyncAll} className="px-4 py-2 border border-gray-400 text-gray-700 hover:bg-gray-100">전체 동기화</button>
                <button onClick={() => setShowManager(true)} className="px-3 py-2 bg-indigo-600 text-white rounded">쇼핑몰별 상품 관리</button>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {malls.map((mall) => (
              <div key={mall.id} onClick={() => setSelectedMall(mall.id)} className={`p-3 border cursor-pointer ${selectedMall === mall.id ? 'border-gray-800 bg-gray-100' : 'border-gray-300 hover:border-gray-400'} ${mall.status === 'inactive' ? 'opacity-50' : ''}`}>
                <div className="text-center">
                  ...
                  <div className={`text-xs mt-1 ${mall.status === 'active' ? 'text-gray-600' : 'text-gray-400'}`}>{mall.status === 'active' ? '연결' : '미연결'}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        {selectedMall ? (
          <>
            <div className="bg-white border rounded-lg p-4 mb-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-gray-900">{selectedMallInfo?.name} 상품 목록</h3>
                <div className="flex gap-2 items-center">
                  <button onClick={()=>setShowExtraInfoModal(true)} className="px-3 py-1 border rounded text-sm">부가 정보 관리</button>
                  <button onClick={()=>setShowCategoryMapModal(true)} className="px-3 py-1 border rounded text-sm">카테고리 매핑</button>
                  <TableExportButton data={exportData} fileName={`${selectedMall || 'products'}-products.xlsx`} />
                </div>
              </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">가격 비교</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">재고 비교</th>
                      ...
                      return (
                        <tr key={product.id} className="hover:bg-gray-50">
                          <td className="px-6 py-4">
                            <div>
                              <div className="flex items-center gap-2">
                                <div className="text-sm font-medium text-gray-900">{effective.name}</div>
                                {((overrides && overrides[product.productId]) || Object.values(overrides || {}).find((o:any)=>o.productId===product.productId)) && (
                                  <div className="text-xs bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded">오버라이드</div>
                                )}
                              </div>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm">
                              <div className={`${effective.price !== (effective.mallPrice || 0) ? 'text-red-600' : 'text-gray-900'}`}>기준: ₩{effective.price.toLocaleString()}</div>
                              <div className={`${effective.price !== (effective.mallPrice || 0) ? 'text-red-600' : 'text-gray-600'}`}>쇼핑몰: ₩{(effective.mallPrice || 0).toLocaleString()}</div>
                              {effective.price !== (effective.mallPrice || 0) && <div className="text-xs text-red-500 mt-1">차이: ₩{Math.abs(effective.price - (effective.mallPrice || 0)).toLocaleString()}</div>}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm">
                              <div className={`${effective.stock !== (effective.mallStock || 0) ? 'text-red-600' : 'text-gray-900'}`}>기준: {effective.stock}개</div>
                              <div className={`${effective.stock !== (effective.mallStock || 0) ? 'text-red-600' : 'text-gray-600'}`}>쇼핑몰: {effective.mallStock || 0}개</div>
                              {effective.stock !== (effective.mallStock || 0) && <div className="text-xs text-red-500 mt-1">차이: {Math.abs(effective.stock - (effective.mallStock || 0))}개</div>}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap"><span className={`inline-flex px-2 py-1 text-xs ${syncInfo.color}`}>{syncInfo.text}</span></td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{product.lastSync}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                              <div className="flex gap-2">
                              <button onClick={() => handleSyncProduct(product.id)} className="text-xs text-blue-600 hover:text-blue-900 border border-blue-300 px-2 py-1 rounded">동기화</button>
                              <a href={product.mallUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-green-600 hover:text-green-900 border border-green-300 px-2 py-1 rounded">보기</a>
                              <button onClick={() => handleDeleteProduct(product.id)} className="text-xs text-red-600 hover:text-red-900 border border-red-300 px-2 py-1 rounded">제거</button>
                              <button onClick={() => setShowEditModal({ productId: product.productId })} className="text-xs text-indigo-600 hover:text-indigo-900 border border-indigo-300 px-2 py-1 rounded">편집</button>
                            </div>
                          </td>
                        </tr>
                      )
        ...
        {showManager && selectedMall && (
          <MallProductManager
            mallId={selectedMall}
            availableProducts={products.map((p) => ({ productId: p.productId, name: p.name, mallProductName: p.mallProductName, mallPrice: p.mallPrice, mallStock: p.mallStock }))}
            onClose={() => {
