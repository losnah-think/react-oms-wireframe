        ...
        try {
          ...
          if (sbUrl && sbKey) {
            try {
              ...
              if (resp.ok) {
                ...
                if (su && su.password_hash) {
                  ...
                  try {
                    ...
                    console.log(
                      "[nextauth] Attempting Supabase Auth password grant",
                      authUrl,
                    );
                    ...
                    if (tokenResp.ok) {
                      ...
                      console.log(
                        "[nextauth] Supabase Auth token result",
                      ...
                      console.warn(
                        "[nextauth] Supabase Auth token request failed",
                        tokenResp.status,
                      );
                    }
                  } catch (eAuth) {
                    console.error(
                      "[nextauth] Supabase Auth password grant error",
                      String(eAuth),
                    );
                  }
                }
              ...
              console.error(
                "[nextauth] Supabase REST primary lookup error",
                String(e2),
              );
            ...
            if (sbUrl2 && sbKey2) {
              ...
              if (resp.ok) {
                ...
                if (u && u.password_hash) {
                  ...
                  console.log(
                    "[nextauth] Supabase fallback bcrypt compare",
                    ok,
                  );
                  if (ok)
                    return {
                ...
                console.warn(
                  "[nextauth] Supabase REST fallback GET failed",
                  resp.status,
                );
              }
            }
...
if (shouldSkipAuth()) {
  _defaultHandler = function devAuthHandler(req: any, res: any) {
    try {
      const parts = Array.isArray(req.query?.nextauth)
        ? req.query.nextauth
        : [];
