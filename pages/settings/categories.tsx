...
const CategoriesPage: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      else setItems([{ id: 'cat-1', name: '의류' }, { id: 'cat-2', name: '가전' }])
    } catch {
      setItems([{ id: 'cat-1', name: '의류' }, { id: 'cat-2', name: '가전' }])
    }
  }, [])

  useEffect(() => {
    try { localStorage.setItem('product_categories', JSON.stringify(items)) } catch {}
  }, [items])
  ...
  return (
    <Container maxWidth="lg" padding="md">
      <div className="mb-6">
        ...
        <div className="text-sm text-gray-500">카테고리를 추가, 수정, 삭제할 수 있습니다. (로컬 저장)</div>
      </div>

      <Card padding="lg">
        <div className="mb-4 flex gap-2">
          <input className="flex-1 px-3 py-2 border rounded" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="새 카테고리명" />
