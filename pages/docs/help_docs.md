...
선택 외부 송신: 선택한 상품을 특정 판매처로 전송할 수 있습니다.
...
CSV 파일 업로드
1. 페이지 목적
외부/내부 판매처에서 받은 상품 CSV 파일을 분석·검증·등록하는 기능입니다.
...
외부 판매처 상품 가져오기
1. 페이지 목적
메이크샵, 카페24, 네이버 스마트스토어, 고도몰5 등 외부 판매처 플랫폼과 연결하여 상품 정보를 가져오는 기능입니다.
...
2. 주요 기능
▸ 판매처 연결
각 판매처은 연결하기 버튼을 통해 인증 및 계정 연동이 필요합니다.


연결된 판매처은 “연결됨” 배지가 표시되며, 상태(연결/미연결/오류)로 구분됩니다.


▸ 상품 가져오기
전체 상품 가져오기: 선택한 판매처의 모든 상품을 불러옵니다.
...
3. 화면 탭 안내
개요: 전체 연결 판매처 수, 총 상품 수, 최근 가져오기 요약.


연결된 판매처: 개별 판매처별 상태, 상품 수, 기능/설정, 통계 확인.
...
입력처(플랫폼): 어떤 판매처/플랫폼을 통해 등록되었는지 표시됩니다.
...
- 선택한 상품을 **엑셀로 저장**, **외부 판매처로 보내기**, **삭제**할 수 있어요.
...
- 외부로 못 보내나요? → 먼저 **해당 판매처을 연결**해야 해요. (연결되지 않음 표시 확인)
...
## 6) 외부 판매처에서 상품 가져오기
이 화면에서 할 수 있는 일
- 카페24, 메이크샵, 네이버 스마트스토어, 고도몰 같은 **외부 판매처의 상품**을 우리 시스템으로 가져와요.

어떻게 해요?
1. 먼저 **판매처 연결**을 해요. (연결됨 표시를 확인)
2. 가져오기 방식 고르기
   - **전체 가져오기**: 해당 판매처의 모든 상품을 가져와요.
...
문제가 생기면
- 연결이 안 돼요? → 먼저 해당 판매처에서 **연결하기**를 다시 해 주세요.
